...
「持续复写」不是一句态度,得有机制托着 —— 否则系统建成那天就是它停止生长的那天。

这套系统里有三个:

### 5.1 复盘 —— 让问题被定期发现

复盘是一个**独立的大区**,不是笔记的附属。它的产出有两种:

- 对**事**的复盘 —— 项目做完了,哪里判断错了
- 对**系统**的复盘 —— 这套结构本身哪里不好用

第二种是关键。三、里那条**从 Review 回指 Input 的箭头**指的就是它:复盘的结论会改写系统本身,而不只是留下一段感想。

### 5.2 版本控制 —— 让复写变得安全

整个 vault 纳入 **git 管理**。这件事在笔记场景里不常见,但它改变的东西很本质:
...
**「不敢改」才是笔记库膨胀的真正原因** —— 怕丢掉过去的想法,就只好不断新建。版本控制把这个顾虑消掉之后,复写才真的可行。

### 5.3 复写 —— 笔记不是写完就冻结

一篇笔记的第一版几乎必然是**摘抄和转述**,那还停在 DIKW 的 I 层。

真正推动它到 K 的,是**过一段时间之后有了新的感悟** —— 在别处读到相关的东西、在项目里踩到了坑、或者换了个角度看懂了。这时候回来重写,写不出来的地方就是当初没真懂的地方。

所以复写的触发点**不是日程,是新认知**。没有新东西就不必动它,有了就该动。

由此这个系统里**笔记的默认状态是「可改」而不是「已完成」**:

- 同一主题不新开第二篇,改原来那篇
- 每次复写留在 git 历史里,旧版本仍可回溯
- 改到自己能不看原文讲清楚为止

这三条合起来才构成迭代:**复盘负责发现问题,版本控制负责让修改无风险,复写负责真正执行。**
