...
**这个项目选的是外部存储这条:** 把当下学到的东西存下来,用以对抗遗忘曲线 —— 不指望脑子记住全部,只要求需要的时候能找回来。同时保留后续**扩展和修改**的余地:存进去的不是终稿,而是可以继续长的东西。

选了这条路,问题也就跟着变了 —— 从「怎么记住」变成「**怎么保证存进去的东西还能被取回来**」。后面大半的设计,都是在回答这个问题。
...
于是笔记不能是写完就封存的档案,它必须**可以被推翻、被重写**。

接着上一条:存储层不能只支持**写入**,还必须支持**修改** —— 只能往里加、不能往回改的系统,存得越多,过期的内容占比就越高。
...
习惯会松、记性会忘、心情会变 —— **靠自觉维持的系统迟早会停**。

所以要定义的不是「怎么记」,而是**信息在系统里怎么流动**:每一层的入口判据、出口判据,以及升级到下一层的条件。后面几节就是这套定义 —— 结构(二、)、判据(三、)、以及让它能自我修正的机制(四、)。
